...
module.exports = {
  ...
  getBoard: async (req, res) => {
    const query =
      "query { boards(ids: 202977424 ) { items(limit: 10) { column_values { id value } } } }";
  ...
  getDolarCurrency: async (req, res) => {
    ...
    try {
      ...
      const data = await page.evaluateHandle(() => {
        const currey = document.querySelectorAll("span.xTimes");
  ...
  getFolios: async (req, res) => {
    ...
    try {
      ...
      if (!getAllFolio)
        throw new Error({ error: "No se pudieron obtener los folios" });
